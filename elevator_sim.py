...
st.caption("배치 전략, 혼잡도 로직, 군집 제어, 그리고 사용자 행동 패턴이 통합된 최종 시뮬레이터입니다.")
...
with st.sidebar:
    ...
    # [부활/통합] 사용자 닫힘 버튼 효율
    button_efficiency = st.slider(
        "🔘 닫힘 버튼 사용 효율 (%)", 
        0, 100, 40, 
        help="탑승객이 닫힘 버튼을 눌러 단축되는 시간의 비율입니다."
    )
...
run_btn = st.button("🚀 통합 정밀 분석 실행", type="primary", use_container_width=True)
...
def calculate_master_time(start_idx, end_idx, placements, logic, cong, is_deliv, eff):
    ...
    # [핵심 로직] 사용자 탑승 여부에 따른 동적 문 개폐 시간 (하행일 때 타고 있다고 가정)
    is_passenger_inside = True if start_idx > idx_1f else False
    if is_passenger_inside:
        actual_door_t = base_door_time * (1 - (eff / 100))
    else:
        actual_door_t = base_door_time * 1.2 # 하차 후 자동 닫힘 지연
    
    wait_t = (min_dist * 1.0) + (1.5 * w)
    move_t = (abs(start_idx - end_idx) * 1.0) + (1.5 * w)
...
if run_btn:
    ...
    def get_random_avg(start, end, n, logic, cong, is_deliv, eff):
        results = [calculate_master_time(start, end, [random.randint(0, total_fs-1) for _ in range(n)], logic, cong, is_deliv, eff)[0] for _ in range(10)]
    ...
    st.subheader(f"📊 통합 분석 결과 (알고리즘: {logic_type})")
    m_cols = st.columns(4)
    report_list, chart_data = [], []

    for i, (name, (start, end, limit)) in enumerate(nodes.items()):
        strategy_t, door_t = calculate_master_time(start, end, final_placements, logic_type, congestion_level, delivery_mode, button_efficiency)
        random_t = get_random_avg(start, end, num_elevators, logic_type, congestion_level, delivery_mode, button_efficiency)
        ...
        report_list.append({"노선": name, "전략 적용": f"{strategy_t:.1f}초", "문 개폐": f"{door_t:.1f}초", "무작위 대비": f"{strategy_t-random_t:+.1f}초", "상태": "안정" if not is_exceed else "위험"})
    ...
    with c_info:
        st.info(f"💡 **행동 분석:** 현재 닫힘 버튼 효율 {button_efficiency}% 적용 중. 탑승 시 문 개폐 시간이 단축되었습니다.")
        st.write(f"- 혼잡 가중치: {curr_w}배")
        st.write(f"- 운영 로직: {logic_type}")
