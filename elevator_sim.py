...
# AI 최적화 또는 운행 규칙이 기본이 아닐 때 시간대 설정
if placement_option == "AI 자동 최적화 배치" or logic_option != "사용 안 함 (전 층 자유 운행)":
    ...
    for i in range(num_elevators):
        with m_cols[i]:
            val = st.selectbox(f"EL {chr(65+i)}", options=range(total_fs), format_func=lambda x: FLOOR_LABELS[x], index=idx_1f, key=f"v_last_final_{i}")
...
with st.container():
    if not show_metric:
        st.info("💡 **알림:** 현재 설정에서는 고정된 대기 위치가 필요하지 않아 배치 상태를 표시하지 않습니다.")
    else:
...
def simulate_route(start, end, placements, logic, cong, is_deliv, eff, base_t, p_rate):
    ...
    # [수정] 주차장(B층) 노선일 경우 이용 비율 가중치 적용
    p_weight = (p_rate / 100) if (start < idx_1f or end < idx_1f) else 1.0
    
    total_t = (wait_t + move_t + (door_eff_t * w)) * (1.3 if is_deliv else 1.0) * p_weight
    return total_t

# ----------------- [5] 결과 확인 및 그래프 -----------------
st.subheader("🌐 환경 설정 및 최종 실행")
...
if st.button("🚀 정밀 시뮬레이션 분석 시작", type="primary", use_container_width=True):
    ...
    for i, (name, (s, e, l)) in enumerate(scenarios.items()):
        # 전략 미적용 (주차장 비율은 동일하게 적용)
        t_base = simulate_route(s, e, control_placements, "자유 운행", congestion, delivery_mode, 0, base_door_time, parking_usage_rate)
        # 전략 적용
        ...
        with m_cols[i]:
            ...
            if t_strat > l: st.error(f"SLA 지연 ({l}s)")
            else: st.success(f"SLA 충족 ({l}s)")
    ...
    df_p['개선 시간(s)'] = df_p['전략 미적용'] - df_p['전략 적용']
    df_p['개선율(%)'] = (df_p['개선 시간(s)'] / df_p['전략 미적용'] * 100).fillna(0)
