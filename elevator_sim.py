...
> * **표준 물리 참조 모델 (Reference Model):** 본 시스템은 기어리스 동기모터(Efficiency 85%) 및 KEPCO 공동주택용 종합계약 단가를 기준으로 설계된 **'표준 물리 참조 모델'**을 사용합니다. 아파트별 절대적인 수치는 다를 수 있으나, 알고리즘 간의 상대적 에너지 절감 효율(%)은 수학적으로 동일하게 유지됩니다.
