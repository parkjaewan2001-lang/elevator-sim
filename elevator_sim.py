...
> * **개별 동선 추적:** 4개 동선의 실시간 소요 시간과 개별 SLA 달성률을 정밀 모니터링합니다.
> * **DES Event-Driven 구조:** 승객 호출 대기열이 쌓이면 엘리베이터가 이벤트를 순차적으로 처리합니다.
> * **시간대별 수요 가중치:** 출근 시간은 `주거층 → 1층/B1`, 퇴근 시간은 `1층/B1 → 주거층` 호출이 높은 확률로 발생합니다.
> * **Queue 지표 추가:** 모든 운영 전략별 평균 대기시간, 최대 대기시간, 평균 Queue 길이를 산출합니다.
> * **대조 분석 기능:** 모든 전략은 **전략 미적용 (랜덤 운행)** 대비 증감률로 자동 환산됩니다.
""")
...
with c_time:
    ...
    current_is_deliv = mode_label == "새벽 시간"
...
def generate_weighted_trip_by_time(mode_label, start_idx, tot_floors, parking_rate, stairs_floor):
    ...
    def pick_lobby_or_parking():
        return parking_floor if random.random() < parking_rate / 100 else lobby_floor
...
def simulate_route_esg_sla_des(
    ...
    for req in requests:
        ...
        wait_times.append(max(0.0, t_arrive - req["t_sp"]))
...
if "demo_timeline_df" not in st.session_state:
    st.session_state.demo_timeline_df = None
if "demo_timeline_mode" not in st.session_state:
    st.session_state.demo_timeline_mode = None

if st.button("🔄 승객 대기열(Queue) 5건 이벤트 타임라인 시뮬레이션", type="secondary"):
    demo_queue = []

    for i in range(5):
        start, end = generate_weighted_trip_by_time(mode_label, idx_1f, total_fs, parking_usage_rate, stairs_floor)
        demo_queue.append(EventRequest(i + 1, random.uniform(0, 60), start, end, random.randint(1, 10)))
    ...
    def fmt_time(seconds):
        return (base_dt + pd.Timedelta(seconds=int(seconds))).strftime("%H:%M:%S")

    st.session_state.demo_timeline_df = pd.DataFrame([
        {
            "호출 ID": f"REQ-{req.req_id}",
            "이동 동선": f"{FLOOR_LABELS[req.start_floor]} → {FLOOR_LABELS[req.end_floor]}",
            "배정 E/V": req.assigned_el,
            "1. 호출 발생": fmt_time(req.t_spawn),
            "2. E/V 배정": fmt_time(req.t_assign),
            "3. 도착(문열림)": fmt_time(req.t_arrive),
            "4. 탑승 완료": fmt_time(req.t_board),
            "5. 목적지 하차": fmt_time(req.t_drop),
            "대기시간": f"{req.t_arrive - req.t_spawn:.1f}초"
        }
        for req in demo_queue
    ])
    st.session_state.demo_timeline_mode = mode_label

if st.session_state.demo_timeline_df is not None:
    st.write(f"##### ⏳ 개별 승객의 시간대별 이벤트 처리 결과: {st.session_state.demo_timeline_mode}")
    st.dataframe(st.session_state.demo_timeline_df, use_container_width=True)
    ...
    for s_name, (start, end, target_sla) in scenarios.items():
        ...
        for strat_name, config in strategies_config.items():
            ...
            if strat_name == "전략 미적용 (랜덤 운행)":
                ...
                placement_text = ", ".join([f"EL {chr(65 + i)}:{FLOOR_LABELS[p]}" for i, p in enumerate(config["placements"])])
...
if st.session_state.strategy_results is not None:
    ...
    for _, row in queue_summary.iterrows():
        ...
        display_name = "AI 배치" if "AI 자동 최적화" in original_name else display_name_map.get(original_name, original_name)
        result_lines.append(
    ...
    for _, row in df_esg_summary.iterrows():
        ...
        esg_rows.append({
            "운영 전략": strat,
            "총 전력 소비량": f"{kwh_v:.4f} kWh" + (f" ({kwh_diff_pct:+.1f}%)" if strat != "전략 미적용 (랜덤 운행)" else " (기준)"),
            "총 예상 전기요금": f"{cost_v:.1f} 원" + (f" ({cost_diff_pct:+.1f}%)" if strat != "전략 미적용 (랜덤 운행)" else " (기준)"),
            "누적 탄소 배출 발자국": f"{co2_v:.1f} g CO₂" + (f" ({co2_diff_pct:+.1f}%)" if strat != "전략 미적용 (랜덤 운행)" else " (기준)")
        })
