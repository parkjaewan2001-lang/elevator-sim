...
with c_custom:
    ...
    for i in range(num_elevators):
        with m_cols[i]:
            val = st.selectbox(f"EL {chr(65+i)}", options=range(total_fs), format_func=lambda x: FLOOR_LABELS[x], index=idx_1f, key=f"v_regen_master_{i}")
...
def simulate_route_esg_sla(start, end, placements, logic, cong, is_deliv, eff, base_t, fixed_t, p_rate, s_floor, households):
    ...
    # 🔄 [회생 제동(Regenerative Braking) 알고리즘 주입 매트릭스]
    # 엘리베이터 카와 균형추(Counterweight)의 중력 시소 원리 메커니즘 해석
    is_upward = (end > start)
    is_heavy_load = (w >= 1.2 or is_deliv)  # 혼잡도가 높거나 배달짐 적재 시 만차로 가정
    
    regen_factor = 1.0
    if is_upward and not is_heavy_load:
        # Case A: 빈 차에 가까운 상태로 상행(⬆️) -> 균형추가 더 무거우므로 중력 자가발전 발생
        regen_factor = -0.35  # 전력 소모가 아닌 약 35% 발전 세이브 효과
    elif not is_upward and is_heavy_load:
        # Case B: 무거운 만차 상태로 하행(⬇️) -> 카가 더 무거우므로 중력 자가발전 발생
        regen_factor = -0.40  # 전력 소모가 아닌 약 40% 발전 세이브 효과
    elif is_upward and is_heavy_load:
        # Case C: 무거운 상태로 상행 -> 모터가 억지로 끌어올려야 하므로 가속 부하 증가
        regen_factor = 1.30
        
    energy_move_final = energy_move_base * regen_factor
    energy_door = 0.001 * w * door_holding_penalty
    
    # 회생제동으로 인해 무부하 주행 시 마이너스 에너지가 누적될 수 있도록 하한선 해제
...
if st.button("🚀 동선별 통합 전략 시뮬레이션 및 그래프 시각화", type="primary", use_container_width=True):
    ...
    # 🌿 [2] 테이블 출력: ESG 환경 부하 통합 스코어보드 (회생제동 반영본)
    st.write("### 🌿 [ESG 친환경 부하 분석] 전략별 누적 에너지 및 탄소 배출 비교 (회생제동 장치 가동)")
